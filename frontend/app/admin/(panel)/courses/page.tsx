...
export default async function AdminCoursesPage() {
  ...
  const { data: courses, error } = await supabase
    .from('courses')
    .select('id, title, level, instructor_name, duration_hours, enrolled_count, category, status')
  ...
  return (
    <div className="px-6 md:px-10 py-8 max-w-[1400px] mx-auto w-full">
      <header className="flex items-end justify-between mb-8 gap-6 flex-wrap">
        <div>
          ...
          <p className="text-text-tertiary mt-1" style={{ fontSize: 14 }}>
            Create and publish courses — they appear on the home page when published.
          </p>
        </div>
      ...
      {error && <p className="text-text-error text-sm mb-4">Failed to load courses: {error.message}</p>}

      <div style={{ borderRadius: 16, overflow: 'hidden', border: '1px solid var(--border)' }}>
        <div className="grid items-center" style={{ gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 160px', background: 'var(--bg-surface)', padding: '16px 24px', gap: 12 }}>
        ...
        {(courses ?? []).map((c) => {
          ...
          return (
            <div key={course.id} className="grid items-center" style={{ gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 160px', padding: '14px 24px', gap: 12, borderTop: '1px solid var(--border)' }}>
              ...
              <div className="flex gap-3 items-center">
                ...
                <AdminDeleteButton label={course.title} onDelete={() => deleteCourseAction(course.id)} />
              </div>
            </div>
          )
        })}
